...
namespace EasyPack.ENekoFramework.Editor
{
    ...
    {
        [Header("生成类型")] [Tooltip("选择要生成的代码类型")]
        public CodeType codeType = CodeType.Service;

        [Header("基本信息")] [Tooltip("类名（例如：PlayerInventory）")]
        public string className = "MyService";

        [Tooltip("命名空间（例如：Game.Services）")] public string namespaceName = "Game.Services";

        [Header("高级选项")] [Tooltip("命令/查询的返回类型")]
        public string resultType = "void";

        [Tooltip("输出路径（相对于 Assets）")] public string outputPath = "Scripts/Generated";
