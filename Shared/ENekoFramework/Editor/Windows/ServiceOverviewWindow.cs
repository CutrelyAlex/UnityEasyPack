...
namespace EasyPack.ENekoFramework.Editor
{
    ...
    public class ServiceOverviewWindow : EditorWindow
    {
        ...
        // 状态颜色
        private readonly Color _uninitializedColor = new(0.7f, 0.7f, 0.7f);
        private readonly Color _initializingColor = new(1f, 0.8f, 0.3f);
        private readonly Color _readyColor = new(0.3f, 1f, 0.3f);
        private readonly Color _pausedColor = new(0.9f, 0.6f, 0.3f);
        private readonly Color _disposedColor = new(1f, 0.3f, 0.3f);
        ...
        private void Update()
        {
            ...
            {
                EditorApplication.delayCall += () => { RefreshServicesAsync(); };
        ...
        private void DrawToolbar()
        {
            ...
            if (GUILayout.Button("刷新", EditorStyles.toolbarButton, GUILayout.Width(60))) RefreshServices();
            ...
            // 监控开关
            bool monitoringEnabled = EditorMonitoringConfig.EnableServiceMonitoring;
            ...
            if (_services != null) GUILayout.Label($"服务总数: {_services.Count}", EditorStyles.toolbarButton);
        ...
        private List<ServiceDescriptor> GetFilteredServices()
        {
            ...
            if (!filterChanged && _cachedFilteredServices != null) return _cachedFilteredServices;

            // 重新计算过滤结果
            if (_services == null || _services.Count == 0)
            {
                _cachedFilteredServices = new();
            }
            else
            {
                ...
                if (currentSelectedArchitectures.Count > 0)
                {
                    ...
                    {
                        string serviceNamespace = s.ServiceType.Namespace;
                ...
                // 状态筛选
                if (_useStateFilter) filtered = filtered.Where(s => s.State == _selectedStateFilter).ToList();
        ...
        private void RefreshArchitectureList()
        {
            ...
            foreach (string arch in architectureNames)
            {
                ...
                _architectureFilters.Add(!previousFilters.TryGetValue(arch, out bool previousFilter) || previousFilter);
            }
        }
        ...
        private void DrawServiceItem(ServiceDescriptor service)
        {
            ...
            // 状态指示器
            Color stateColor = GetStateColor(service.State);
            ...
            // 服务名称
            if (GUILayout.Button(service.ServiceType.Name, EditorStyles.label)) _selectedService = service;
        ...
        private void DrawMetadata()
        {
            ...
            ServiceInspector.ServiceMetadata metadata = ServiceInspector.GetServiceMetadata(_selectedService);

            if (metadata != null)
            {
                EditorGUILayout.LabelField("依赖数量", metadata.Dependencies?.Count.ToString() ?? "0");

                if (metadata.IsCircularDependency) EditorGUILayout.HelpBox("⚠️ 此服务存在循环依赖！", MessageType.Warning);
            }
        }
        ...
        private void RefreshServicesAsync()
        {
            ...
            System.Threading.Tasks.Task.Run(() =>
            {
                ...
                {
                    // 异常处理：确保UI状态正确重置
                    Debug.LogError($"ServiceOverviewWindow: 刷新服务列表时发生异常 - {ex.Message}\n{ex.StackTrace}");
        ...
        private List<ServiceDescriptor> RefreshServicesInternal() => ServiceInspector.GetAllServices();
