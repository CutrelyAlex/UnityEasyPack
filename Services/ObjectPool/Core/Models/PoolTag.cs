...
namespace EasyPack.ObjectPool
{
    ...
    public readonly struct PoolTag : IEquatable<PoolTag>, IComparable<PoolTag>
    {
        ...
        ///     默认标记，适用于未显式指定配置的池。
        /// </summary>
        public static PoolTag Default => new(0);
        ...
        private PoolTag(ulong value) => Value = value;
        ...
        {
            ushort typeHash = (ushort)(typeof(T).GetHashCode() & 0xFFFF);
            ulong configHash = unchecked((ulong)(configKey?.GetHashCode() ?? 0)) & ConfigMask;
            ulong packed = ((ulong)typeHash << (64 - TypeBits)) | configHash;
            return new(packed);
        }
        ...
        {
            ulong packed = ((ulong)typeDiscriminator << (64 - TypeBits)) | (configDiscriminator & ConfigMask);
            return new(packed);
        }
        ...
        ///     从现有值恢复标记。
        /// </summary>
        public static PoolTag FromValue(ulong value) => new(value);
        ...
        public override string ToString() =>
            $"PoolTag(Type:0x{GetTypeDiscriminator():X4}, Config:0x{GetConfigDiscriminator():X12})";
