...
namespace EasyPack.ObjectPool
{
    ...
    public abstract class PoolManagerBase
    {
            ...
            int maxCapacity = 64) where T : class
        {
            ...
            var poolsByType = _pools.GetOrAdd(typeKey, _ => new ConcurrentDictionary<ulong, object>());
        ...
        {
            var typeKey = typeof(T);
            if (!_pools.TryGetValue(typeKey, out var poolsByType))
            {
                return null;
            }

            var tag = PoolTag.Create<T>(configKey);
            return poolsByType.TryGetValue(tag.Value, out var pool)
        ...
        protected bool DestroyPool<T>(string configKey) where T : class
        {
            ...
            var tag = PoolTag.Create<T>(configKey);
            if (!poolsByType.TryRemove(tag.Value, out var pool))
            {
                return false;
            }
            ...
            {
                _pools.TryRemove(typeKey, out _);
            }
            return true;
        }
