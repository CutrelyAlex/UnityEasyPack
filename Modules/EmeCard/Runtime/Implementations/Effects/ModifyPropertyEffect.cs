...
namespace EasyPack.EmeCardSystem
{
    ...
    ///     说明：
    ///     <para></para>
    ///     <see cref="ApplyMode" /> 应用模式
    ///     <para></para>
    ///     <see cref="Modifier" /> 要应用在属性上的修饰器
    ///     <para></para>
    ///     <see cref="PropertyName" /> 要应用的属性名称
    ///     <para></para>
    /// </summary>
                ...
                foreach (GameProperty gp in propsToModify)
                {
                    ...
                    switch (ApplyMode)
                    {
                        case Mode.AddModifier:
                            if (Modifier != null) gp.AddModifier(Modifier);

                            break;
                        case Mode.RemoveModifier:
                            if (Modifier != null) gp.RemoveModifier(Modifier);
