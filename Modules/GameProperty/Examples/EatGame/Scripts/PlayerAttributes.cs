...
namespace EasyPack.GamePropertySystem.Example.EatGame
{
    ...
    public class PlayerAttributes
    {
        ...
        {
            // 初始化一级属性
            Satiety = new("Satiety", 50f);
            Health = new("Health", 100f);
            Sanity = new("Sanity", 100f);

            // 初始化二级属性
            SatietyChangePerDay = new("SatietyChangePerDay", -10f); // 每天减少10饱食度
            HealthChangePerDay = new("HealthChangePerDay", 0f);
            SanityChangePerDay = new("SanityChangePerDay", 0f);
            SatietyCapacity = new("SatietyCapacity", 100f);
            HealthCapacity = new("HealthCapacity", 100f);
            SanityCapacity = new("SanityCapacity", 100f);

            // 初始化奇怪评分 - 使用依赖系统实现复杂组合计算
            StrangeScore = new("StrangeScore", 0f);
        }
        ...
        public void ProcessDailyChanges()
        {
            ...
            Debug.Log(
                $"应用每日变化 - 饱食度: {Satiety.GetValue():F1} + ({SatietyChangePerDay.GetValue():+0.0;-0.0;0}) = {Mathf.Clamp(Satiety.GetValue() + SatietyChangePerDay.GetValue(), 0, SatietyCapacity.GetValue()):F1}");
            Debug.Log(
                $"应用每日变化 - 生命值: {Health.GetValue():F1} + ({HealthChangePerDay.GetValue():+0.0;-0.0;0}) = {Mathf.Clamp(Health.GetValue() + HealthChangePerDay.GetValue(), 0, HealthCapacity.GetValue()):F1}");
            Debug.Log(
                $"应用每日变化 - SAN值: {Sanity.GetValue():F1} + ({SanityChangePerDay.GetValue():+0.0;-0.0;0}) = {Mathf.Clamp(Sanity.GetValue() + SanityChangePerDay.GetValue(), 0, SanityCapacity.GetValue()):F1}");

            // 应用每日变化
            Satiety.SetBaseValue(Mathf.Clamp(Satiety.GetValue() + SatietyChangePerDay.GetValue(), 0,
                SatietyCapacity.GetValue()));
            Health.SetBaseValue(Mathf.Clamp(Health.GetValue() + HealthChangePerDay.GetValue(), 0,
                HealthCapacity.GetValue()));
            Sanity.SetBaseValue(Mathf.Clamp(Sanity.GetValue() + SanityChangePerDay.GetValue(), 0,
                SanityCapacity.GetValue()));
        }

        /// <summary>
        ///     检查游戏结束条件
        /// </summary>
        public bool IsGameOver() => Health.GetValue() <= 0 || Satiety.GetValue() <= 0 || Sanity.GetValue() <= 0;
        ...
        {
            string status =
                $"饱食度: {Satiety.GetValue():F1}/{SatietyCapacity.GetValue()} ({SatietyChangePerDay.GetValue():+0.0;-0.0;0}/天)\n" +
            ...
            if (!string.IsNullOrEmpty(statusEffects)) status += "\n\n" + statusEffects;
        ...
        private string GetStatusEffectsDescription()
        {
            ...
            if (satiety > 80) effects.Add("⚠️ 饱食度过高：SAN值每日额外减少3点");
