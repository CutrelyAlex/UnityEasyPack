...
namespace EasyPack.GamePropertySystem.Example.EatGame
{
    ...
    public class EatGameController : EasyPackController
    {
        ...
        private void InitializeGame()
        {
            ...
            if (confirmButton != null) confirmButton.onClick.AddListener(ConfirmEat);

            if (cancelButton != null) cancelButton.onClick.AddListener(CancelSelection);

            if (nextDayButton != null) nextDayButton.onClick.AddListener(NextDay);
        ...
        public void ConfirmEat()
        {
            ...
            if (selectedFood.SustainedEffects.Count > 0)
            {
                ...
                foreach (var effect in selectedFood.SustainedEffects)
                {
                    ...
                    Debug.Log(
                        $"  {attrName}: {sign}{effect.Value.value} (持续{effect.Value.duration}天, 类型:{effect.Value.type})");

                    // 详细显示modifier应用逻辑
                    Debug.Log(
                        $"    Modifier详情: 类型={effect.Value.type}, 值={effect.Value.value}, 持续时间={effect.Value.duration}天");
        ...
        public void NextDay()
        {
            ...
            Debug.Log("\n--- 每日属性结算 ---");
            Debug.Log(
                $"结算前每日变化: 饱食度{playerAttributes.SatietyChangePerDay.GetValue():+0.0;-0.0;0}, 生命值{playerAttributes.HealthChangePerDay.GetValue():+0.0;-0.0;0}, SAN{playerAttributes.SanityChangePerDay.GetValue():+0.0;-0.0;0}");
        ...
        {
            if (statusText) statusText.text = playerAttributes.GetStatusDescription();

            if (strangeScoreText) strangeScoreText.text = $"奇怪评分: {playerAttributes.StrangeScore.GetValue():F1}/100";
        }
